export default {
  ...
  dataset: {
    ...
    select_year: '选择年',
    sql_variable_limit_1: '1、SQL 变量只能在 WHERE 条件、MySQL 存储过程、PostgreSQL 函数、SQL Server 存储过程中使用，需要注意的是存储过程/函数的参数为单个参数则用单引号包裹，如\'${param_name1}\'，参数为数组则用双引号包裹，如"${param_name2}"',
    sql_variable_limit_2: '2、SQL 示例：select * from table_name where column_name1=\'${param_name1}\' and column_name2 in ${param_name2}',
    sql_variable_limit_3: '3、MySQL 存储过程示例：CALL sp_financial_analysis_simple(\'${param_name1}\',"${param_name2}")',
    sql_variable_limit_4: '4、PostgreSQL 函数示例：SELECT * FROM sp_financial_analysis_simple(\'${param_name1}\',"${param_name2}")',
    ...
    parse_error: 'Excel解析失败，请检查格式、字段等信息。以下为导入Excel的规范准则：' +
      '1.文件中不能存在合并单元格；' +
  ...
  datasource: {
    ...
    auth_method: '认证方式',
    passwd: '用户名密码',
    kerbers_info: '请确保 krb5.Conf、Keytab Key，已经添加到路径：/BITools/Server/resource/conf',
