export default {
  ...
  commons: {
    ...
    thumbnail: '縮略圖',
    confirm_delete: '確認刪除',
    delete_this_dashboard: '確認刪除該儀錶板嗎?',
    cancel_this_dashboard: '確認取消該默認儀錶板嗎?',
  ...
  dataset: {
    scope_edit: '僅編輯時生效',
    scope_all: '數據集預覽時全域生效',
    spend_time: '耗時',
    sql: 'SQL 語句',
    sql_result: '運行結果',
    parse_filed: '解析字段',
    field_rename: '字段重命名',
    params_work: '僅編輯時生效：參數值僅在數據集編輯時生效； 全域生效：在數據集查看、預覽、以及用到數據集的視圖中均生效。',
    sql_variable_limit_1: '1、SQL 變量只能在 WHERE 條件、MySQL 存儲過程、PostgreSQL 函數、SQL Server 存儲過程中使用，需要注意的是存儲過程/函數的參數爲單個參數則用單引號包裹，如\'${param_name1}\'，參數爲數組則用雙引號包裹，如"${param_name2}"',
    sql_variable_limit_2: '2、SQL 示例：select * from table_name where column_name1=\'${param_name1}\' and column_name2 in ${param_name2}',
    sql_variable_limit_3: '3、MySQL 存儲過程示例：CALL sp_financial_analysis_simple(\'${param_name1}\',"${param_name2}")',
    sql_variable_limit_4: '4、PostgreSQL 函數示例：SELECT * FROM sp_financial_analysis_simple(\'${param_name1}\',"${param_name2}")',
    sql_variable_limit_5: '5、SQL Server 存儲過程示例：EXEC sp_financial_analysis_simple \'${param_name1}\',"${param_name2}"',
    ...
    parse_error: 'Excel解析失敗，請檢查格式、字段等信息。以下為導入Excel的規範準則：' +
      '1.文件中不能存在合併儲存格；' +
      '2.文件的第一行為標題行，不能為空，不能為日期型；' +
      '3.Excel 檔案大小請確保在 500M 以內。',
  ...
  datasource: {
    ...
    auth_method: '認證方式',
    passwd: '用戶名密碼',
    kerbers_info: '請確保 krb5.Conf、Keytab Key，已經添加到路徑：/BITools/Server/resource/conf',
    ...
    cannot_be_deleted_dataset: '該數據集存在如下血緣關係，刪除會造成相關儀錶板的視圖失效，確定刪除？',
    cannot_be_deleted_datasource: '該數據源存在如下血緣關係，刪除會造成相關儀錶板的視圖失效，確定刪除？',
    edit_folder: '編輯資料夾',
  ...
  panel: {
    ...
    panel_background_image_tips: '當前支持jpeg,jpg,png,gif,svg文件,大小15M內',
    reUpload: '重新上傳',
    create_by: '創建人',
    create_time: '創建時間',
    update_by: '最近修改人',
